...
class AccountRepository {
  ...
  Future<List<Account>> getAccountsIncludedInTotal() async {
    try {
      ...
      return allAccounts.where((account) => 
          account.isActive && account.includeInTotal).toList();
    } catch (e) {
      throw DatabaseException(message: 'Failed to get accounts included in total: $e');
    }
  }
  ...
  Future<void> subtractFromAccountBalance(String accountId, double amount) async {
    try {
      ...
      if (e is AccountNotFoundException || e is InsufficientFundsException) rethrow;
      throw DatabaseException(message: 'Failed to subtract from account balance: $e');
    }
  }
  ...
  // Get total balance
  Future<double> getTotalBalance({String currency = 'USD'}) async {
    try {
      final accounts = await getAccountsIncludedInTotal();
      ...
      for (final account in accounts) {
        ...
        // TODO: Convert different currencies using exchange rates
      }
      
      return total;
    } catch (e) {
      throw DatabaseException(message: 'Failed to get total balance: $e');
    }
  }
  ...
  Future<Account> _encryptAccountIfNeeded(Account account) async {
    ...
    final encryptedBalance = await _encryptionService.encryptDouble(account.balance);
  ...
  Future<Account> _decryptAccountIfNeeded(Account account) async {
    ...
    final encryptedBalance = account.metadata?['encrypted_balance'] as String?;
    final encryptedAccountNumber = account.metadata?['encrypted_account_number'] as String?;
